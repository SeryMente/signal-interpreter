...
internal sealed class LocalTransport : IDisposable
{
    ...
    public LocalTransport(int port) { _port = port; _listener = new TcpListener(IPAddress.Loopback, _port); }
    ...
    public void Publish(object value)
    {
        ...
        foreach (var client in _clients.Values) client.TryQueue(json);
    }
    ...
    private async Task HandleClientAsync(TcpClient tcpClient)
    {
        using (tcpClient)
        {
            try
            {
                ...
                {
                    await SendHttpResponseAsync(stream, 200, "application/json; charset=utf-8",
                        JsonSerializer.Serialize(new
                        {
                            service = "signal-live-caption",
                ...
                {
                    await SendHttpResponseAsync(stream, 400, "text/plain; charset=utf-8", "WebSocket upgrade required.", _cts.Token);
                    return;
                }
                ...
                try
                {
                    while (!_cts.IsCancellationRequested && await connection.ReceiveAsync()) { }
                }
                finally
                {
    ...
    private async Task HeartbeatLoopAsync()
    {
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(_cts.Token))
                Publish(new { type = "bridge.heartbeat", protocol = "signal-live-caption.v1", timestamp = DateTimeOffset.UtcNow, clients = _clients.Count });
        }
    ...
    private static async Task SendHttpResponseAsync(Stream stream, int status, string contentType, string body, CancellationToken token)
    {
    ...
    public void Dispose()
    {
        ...
        foreach (var client in _clients.Values) _ = client.DisposeAsync();
        _clients.Clear();
        _started = false;
    }
    ...
    private sealed class ClientConnection
    {
        ...
        private async Task SendTextAsync(string json)
        {
            ...
            try
            {
                ...
                try { await WriteFrameAsync(_stream, 0x1, payload, _token); await _stream.FlushAsync(_token); }
        ...
        {
            var header = await ReadExactlyAsync(_stream, 2, _token);
            ...
            ulong length = lengthCode <= 125 ? (ulong)lengthCode :
                lengthCode == 126 ? ReadUInt16BigEndian(await ReadExactlyAsync(_stream, 2, _token)) :
                ReadUInt64BigEndian(await ReadExactlyAsync(_stream, 8, _token));
            ...
            var payload = length == 0 ? Array.Empty<byte>() : await ReadExactlyAsync(_stream, checked((int)length), _token);
        ...
        private static async Task WriteFrameAsync(Stream stream, byte opcode, byte[] payload, CancellationToken token)
        {
            ...
            if (payload.Length <= 125) header.Add((byte)payload.Length);
            else if (payload.Length <= ushort.MaxValue)
            {
                header.Add(126); header.Add((byte)(payload.Length >> 8)); header.Add((byte)payload.Length);
            }
            else
            {
                header.Add(127);
                var length = (ulong)payload.Length;
                for (var i = 7; i >= 0; i--) header.Add((byte)(length >> (8 * i)));
            }
        ...
        {
            if (bytes is null || bytes.Length != 2) throw new IOException("Unexpected WebSocket frame length.");
            return (ushort)((bytes[0] << 8) | bytes[1]);
        }

        private static ulong ReadUInt64BigEndian(byte[]? bytes)
        {
            if (bytes is null || bytes.Length != 8) throw new IOException("Unexpected WebSocket frame length.");
            ulong value = 0; foreach (var b in bytes) value = (value << 8) | b; return value;
        }
    }
}